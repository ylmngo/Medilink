...
func (rm *ReminderModel) Insert(reminder *Reminder) error {
	query := `INSERT into reminders (user_id, appointment, email, note, sent) VALUES ($1, $2, $3, $4, $5) RETURNING reminders_id`
...
func (rm *ReminderModel) GetAllUnsentReminders() ([]Reminder, error) {
	...
	query := `SELECT * from reminders where sent = ($1)`
...
func (rm *ReminderModel) UpdateReminderSent(id int) error {
	query := `UPDATE reminders set sent = TRUE where reminders_id = ($1)`
